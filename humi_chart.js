...
setTimeout(() => {
    $.getJSON("	https://io.adafruit.com/api/v2/Jackson25092002/feeds/bbc-humi/data", function (data) {
        console.log(data);
        for (var i = 0; i < 6; i++) {
            dataHumi.push(+data[i].value);
            var q = new Date(data[i].created_at);
            label_humi.push(((q.getHours() < 10) ? "0" : "") + q.getHours() + ":" + ((q.getMinutes() < 10) ? "0" : "") + q.getMinutes());
        }
        dataHumi.reverse();
        label_humi.reverse();
    });
    ...
    console.log(label_humi);
    a.update();
},1);
document.addEventListener('DOMContentLoaded', function () {
    var ctx = document.getElementById('myChartHumi').getContext('2d');
    var gradient = ctx.createLinearGradient(0, 0, 0, 225);
    gradient.addColorStop(0, 'rgba(255, 177, 103, 0.9)');
    gradient.addColorStop(1, 'rgba(215, 227, 244, 0.7)');
    // Line chart
    a = new Chart(document.getElementById('myChartHumi'), {
        type: 'line',
        data: {
            labels: label_humi,
            datasets: [
...
setInterval(() => {    
    ...
    if ((currentTime.getMinutes() % 2 == 0 && (((currentTime.getHours() < 10) ? "0" : "") + currentTime.getHours() + ":" + ((currentTime.getMinutes() < 10) ? "0" : "") + currentTime.getMinutes()) != label_humi[5])
        || !(currentTime.getHours() == label_humi[5].substring(0, 2) && currentTime.getMinutes() - label_humi[5].substring(3) <= 5)) {
        label_humi.shift();
        label_humi.push(((currentTime.getHours() < 10) ? "0" : "") + currentTime.getHours() + ":" + ((currentTime.getMinutes() < 10) ? "0" : "") + currentTime.getMinutes());
    }
    
    $.getJSON("	https://io.adafruit.com/api/v2/Jackson25092002/feeds/bbc-humi/data", function (data) {
        dataHumi[5] = data.last_value;
...
/* function updateCharts() {
    $.getJSON("	https://io.adafruit.com/api/v2/Jackson25092002/feeds/bbc-humi/data", function (data) {
        ...
        humidityData.push(temp);
        HumidityLabels.push(time);
        if (humidityDataData.length > 10) {
            humidityData.shift();
